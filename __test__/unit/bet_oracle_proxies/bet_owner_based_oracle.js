...
describe('Bet Owner Based Oracle Proxy', () => {
  ...
  before(async () => {
    ...
    await guesser.contracts.betOwnerBasedOracle.setBetRegistry(
  ...
  it('should tell that the outcome is not ready', async () => {
    const outcomeReady = await guesser.contracts.betOwnerBasedOracle.outcomeReady(
        betHash,
        accounts[0],
    );
    expect(
  ...
  it('should tell that the outcome is ready', async () => {
    await guesser.contracts.betOwnerBasedOracle.setOutcomeReady(
      betHash,
      true,
      accounts[0],
    );
    expect(
      await guesser.contracts.betOwnerBasedOracle.outcomeReady(
        betHash,
      ),
    ).to.be.equal(true);
  });
  ...
  it('shoud tell the outcome of the oracle', async () => {
    await guesser.contracts.betOwnerBasedOracle.setOutcome(
      betHash,
      1,
      accounts[0],
    );

    const outcome = await guesser.contracts.betOwnerBasedOracle.getOutcome(
        betHash,
    );
    expect(
      outcome.toNumber(),
